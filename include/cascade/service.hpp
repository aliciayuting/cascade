...
#define CASCADE_NOTIFICATION_MESSAGE_TYPE   (0x100000000ull)
    ...
    /***
     * Exposing API from derecho replicated.hpp
     */
    template <typename SubgroupType>
    void oob_remote_write_proxy(const uint32_t subgroup_index,
    ...
    template <typename SubgroupType>
    void oob_remote_read_proxy(const uint32_t subgroup_index,
            const node_id_t& remote_node,
            const struct iovec* iov, int iovcnt,
            uint64_t remote_src_addr, uint64_t rkey, size_t size);

    template <typename SubgroupType>
    void wait_for_oob_op_proxy(const node_id_t& remote_node, uint32_t op, uint64_t timeout_us);
