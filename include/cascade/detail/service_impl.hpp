...
template <typename... CascadeTypes>
template <typename SubgroupType>
void ServiceClient<CascadeTypes...>::oob_remote_write_proxy(const uint32_t subgroup_index,
            const node_id_t& remote_node,
            const struct iovec* iov, int iovcnt,
            uint64_t remote_dest_addr, uint64_t rkey, size_t size){
    if (!is_external_client()) {
        auto& subgroup_handle = group_ptr->template get_subgroup<SubgroupType>(subgroup_index);
        subgroup_handle.oob_remote_write(remote_node, iov, iovcnt,
                                         remote_dest_addr, rkey, size);
        std::cout << "SENT OOB REMOTE WRITE RPC CALL to node of id:" << remote_node << std::endl;
    }
}

template <typename... CascadeTypes>
template <typename SubgroupType>
void ServiceClient<CascadeTypes...>::oob_remote_read_proxy(const uint32_t subgroup_index,
            const node_id_t& remote_node,
            const struct iovec* iov, int iovcnt,
            uint64_t remote_src_addr, uint64_t rkey, size_t size) {
    if (!is_external_client()) {
        auto& subgroup_handle = group_ptr->template get_subgroup<SubgroupType>(subgroup_index);
        subgroup_handle.oob_remote_read(remote_node, iov, iovcnt,
                                        remote_src_addr, rkey, size);
        std::cout << "SENT OOB REMOTE READ RPC CALL to node of id:" << remote_node << std::endl;
    }
}

template <typename... CascadeTypes>
template <typename SubgroupType>
void ServiceClient<CascadeTypes...>::wait_for_oob_op_proxy(const node_id_t& remote_node, uint32_t op, uint64_t timeout_us){
    if (!is_external_client()) {
        ...
        std::cout << "SENT OOB WAIT RPC CALL to node of id:" << remote_node << std::endl;
    }
}
