...
template <typename T>
void SubgroupNotificationHandler<SubgroupType>::initialize(derecho::ExternalClientCaller<SubgroupType, T>& subgroup_caller) {
...
template <typename SubgroupType>
void SubgroupNotificationHandler<SubgroupType>::operator()(const derecho::NotificationMessage& msg) {
    ...
    mutils::deserialize_and_run(nullptr, msg.body,
                                [this](const CascadeNotificationMessage& cascade_message) -> void {
                                    ...
                                    if(object_pool_notification_handlers.find("") != object_pool_notification_handlers.cend()) {
                                    ...
                                    if(object_pool_notification_handlers.find(cascade_message.object_pool_pathname) != object_pool_notification_handlers.cend()) {
...
std::vector<node_id_t> ServiceClient<CascadeTypes...>::type_recursive_get_shard_members(
        uint32_t type_index, uint32_t subgroup_index, uint32_t shard_index) const {
    if(type_index == 0) {
...
std::tuple<uint32_t, uint32_t, uint32_t> ServiceClient<CascadeTypes...>::key_to_shard(
    ...
    return std::tuple<uint32_t, uint32_t, uint32_t>{opm.subgroup_type_index, opm.subgroup_index,
                                                    opm.key_to_shard_index(key, affinity_set,
                                                                           get_number_of_shards(opm.subgroup_type_index, opm.subgroup_index),
                                                                           check_object_location)};
}
...
inline std::string ServiceClient<CascadeTypes...>::ObjectPoolMetadataCacheEntry::to_affinity_set(
    ...
    if(key_string.size() > 0 && this->opm.affinity_set_regex.size() > 0) {
        ...
        hs_scan(database, key_string.c_str(), key_string.size(), HS_FLAG_SOM_LEFTMOST, scratch, [](unsigned int /*id*/, unsigned long long from, unsigned long long to, unsigned int /*flags*/, void* ctxt) -> int {
